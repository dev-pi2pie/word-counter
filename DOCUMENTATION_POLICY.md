...
Repository-wide policy for guides, notes, schemas, plans, research docs, job records, and related documentation lifecycle decisions.
...
docs/
  guides/
    <short-title>-usage-guide.md
  notes/
    <short-title>-notes.md
  schemas/
    <short-title>.md
  researches/
...
- guide docs, note docs, and schema/contract docs are current reference surfaces and do not get archive subdirectories in the current policy
- job records should remain in `docs/plans/jobs/` even when related research or plan docs are archived
- do not create `docs/guides/archive/`, `docs/notes/archive/`, or `docs/schemas/archive/` in the first documentation reorganization pass
...
- guide docs, note docs, schema/contract docs, and other user-facing reference docs should prefer non-archived current docs over archived ones
- if a guide, note, or schema/contract doc still mentions an archived doc, label it clearly as historical context or historical reference
- historical labeling is required only for guide docs, note docs, schema/contract docs, and other user-facing reference docs, not for archived docs or job-record traceability links
...
Use guide docs for current user-facing usage guidance. A guide should explain how to use a supported command, feature, or workflow.

Location:

```text
docs/guides/<short-title>-usage-guide.md
```

Notes:

- Keep guide docs focused on the current shipped or accepted reader-facing contract.
- Use `modified-date` when a later substantive update changes documented behavior, status, or reader-facing guidance.
- Do not update `modified-date` for routine movement, link-only rewrites, or formatting-only maintenance unless the reader-facing meaning changes.
...
Use note docs for current explanatory material that is not direct usage guidance or a formal schema/contract. Notes may track compatibility intent, release verification, migration context, operational caveats, or decision context.

Location:

```text
docs/notes/<short-title>-notes.md
```

Notes:

- Keep notes focused on current reference context unless they explicitly point to historical research or plans.
- Prefer a guide when the document primarily teaches command or feature usage.
- Prefer a schema/contract doc when the document primarily defines structured output, config, or debug/event shape.
- Use `modified-date` when a later substantive update changes the documented context, status, or reader-facing guidance.
- Do not update `modified-date` for routine movement, link-only rewrites, or formatting-only maintenance unless the reader-facing meaning changes.
...
Note status guidance:

- `draft` - the note is being drafted, or the documented context is not settled
- `active` - the note tracks ongoing compatibility, release, migration, or operational context
- `completed` - the note captures current accepted context and does not require active follow-up
- `blocked` - the note should be completed, but is waiting on a decision or dependency
- `cancelled` - the note was intentionally stopped and should not be used as guidance
- `superseded` - a newer note, guide, schema/contract, research doc, or plan doc replaced this note as the primary reference

### Schema and Contract Documents

Use schema and contract docs for current structured reference material, including output contracts, config defaults, debug/event streams, detector payloads, and other stable shape definitions.

Location:

```text
docs/schemas/<short-title>.md
```

Notes:

- Keep schema/contract docs focused on the current shipped or accepted structured contract.
- Prefer a guide when examples and usage flow are the primary purpose.
- Prefer a note when explanatory or compatibility context is the primary purpose.
- Use `modified-date` when a later substantive update changes documented fields, semantics, defaults, or compatibility guidance.
- Do not update `modified-date` for routine movement, link-only rewrites, or formatting-only maintenance unless the reader-facing meaning changes.
...
```yaml
---
title: "<schema or contract title>"
created-date: YYYY-MM-DD
modified-date: YYYY-MM-DD # optional
status: draft | completed | blocked | cancelled | superseded
agent: <agent name>
...
Schema and contract status guidance:

- `draft` - the contract is being drafted, or the documented shape is not settled
- `completed` - the document defines the current shipped or accepted structured contract
- `blocked` - the contract should be completed, but is waiting on a decision or dependency
- `cancelled` - the contract was intentionally stopped and should not be used as guidance
- `superseded` - a newer schema/contract doc, guide, note, research doc, or plan doc replaced this doc as the primary reference
