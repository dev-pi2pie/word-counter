...
- [ ] Add a dedicated `doctor` subcommand in `src/command.ts` rather than another standalone top-level flag.
- [ ] Keep doctor wiring isolated from count execution so existing count flows and standalone `--print-jobs-limit` validation remain unchanged.
- [ ] Add a small doctor module boundary, proposed:
  - `src/cli/doctor/run.ts`
  - `src/cli/doctor/checks.ts`
  - `src/cli/doctor/render.ts`
  - `src/cli/doctor/types.ts`
- [ ] Define doctor-local option parsing for:
  - default human-readable output
  - `--format json`
  - `--pretty` as JSON indentation control
- [ ] Explicitly constrain the doctor option surface so v1 accepts only its intended diagnostics options:
...
- [ ] Decide how doctor invalid usage maps to exit code `1` while preserving Commander-style error output.

### Phase 2 - Runtime and Capability Checks

- [ ] Implement runtime summary fields:
  - `packageVersion`
  - `buildChannel`
  - `requiredNodeRange`
  - `nodeVersion`
  ...
  - `platform`
  - `arch`
- [ ] Reuse embedded-version sourcing instead of reading `package.json` at doctor runtime.
- [ ] Add a simple build-channel derivation (`stable` / `canary`) from the embedded version string.
- [ ] Add an explicit Node support-policy check against the current project contract (`>=20`).
- [ ] Implement `Intl.Segmenter` checks for:
  - availability
  - word granularity constructor health
  - grapheme granularity constructor health
  - sample segmentation iteration without throwing
- [ ] Treat missing or unusable `Intl.Segmenter` as the primary `fail` condition.

### Phase 3 - Jobs and Worker Diagnostics

- [ ] Reuse `resolveBatchJobsLimit()` for doctor jobs diagnostics so `doctor` and `--print-jobs-limit` stay aligned.
- [ ] Report jobs diagnostics fields:
  - `cpuLimit`
  - `uvThreadpool`
  - `ioLimit`
  - `suggestedMaxJobs`
- [ ] Report worker-route env toggles:
  - `WORD_COUNTER_DISABLE_WORKER_JOBS`
  - `WORD_COUNTER_DISABLE_EXPERIMENTAL_WORKERS`
- [ ] Add worker preflight signals that map to real fallback paths in current code:
  - `workerThreadsAvailable`
  - `workerRouteDisabledByEnv`
  - `workerPoolModuleLoadable`
  - `workerEntryFound`
- [ ] Refactor or expose any worker preflight helper needed so doctor does not duplicate fragile worker-entry resolution logic.
- [ ] Keep doctor side-effect free by avoiding real worker task execution.

### Phase 4 - Rendering, Status, and Exit Codes

- [ ] Add human-readable doctor rendering with sectioned output for:
  - runtime
  - segmenter
  - batch jobs
  - worker route
- [ ] Use `picocolors` for human-readable emphasis when color is supported.
- [ ] Keep JSON output uncolored and stable.
- [ ] Keep JSON formatting aligned with the existing CLI contract:
  - `--format json` prints compact JSON
  - `--format json --pretty` prints indented JSON
- [ ] Lock the v1 JSON payload shape to the researched contract, including:
  - top-level `status`
  - `runtime`, `segmenter`, `jobs`, and `workerRoute` sections
  - `warnings` as a stable top-level array
- [ ] Implement top-level status reduction:
...
- [ ] Add command tests in `test/command.test.ts` for:
...
- [ ] Add explicit assertions for doctor exit-code contracts:
  - `0` for `ok`
  - `0` for `warn`
  - `1` for invalid doctor usage
  - `2` for runtime `fail`
- [ ] Add payload-shape tests for the researched JSON contract so field names and section layout do not drift during implementation.
- [ ] Add coverage for jobs and worker diagnostics parity with current helpers and env toggles.
- [ ] Add tests that doctor remains isolated from counting inputs and batch path flags.
- [ ] Add regression tests that existing root CLI behavior remains intact after introducing the subcommand:
...
- [ ] Update `README.md` with doctor usage examples and JSON-formatting expectations.
- [ ] Update any existing diagnostics docs that currently point only to `--print-jobs-limit` as the future bridge.
- [ ] Record implementation work in follow-up job records as phases are completed.
